...
@pytest.mark.parametrize(
    ...
    [
        [  # ts0 - load v1.0 table with "->" without units
            {"path": "flowcx.float.df.pkl"}, "ref.flowcx.float.df.pkl"
        ],
        [  # ts1 - load v1.0 table with "->" and with "->" units
            {"path": "flowcx.units.float.df.pkl"}, "ref.flowcx.units.float.df.pkl"
        ],
        [  # ts2 - roundtrip table with flat columns and units
            {"path": "Ewe_I.df.pkl"}, "Ewe_I.df.pkl"
        ],
        [  # ts3 - load v1.0 table with a mixture of "->"
            {"path": "2022_02_22.df.GC+flow+I.pkl"}, "ref.df.GC+flow+I.pkl"
        ],
        [  # ts4 - roundtrip test
            {"path": "ref.df.GC+flow+I.pkl"}, "ref.df.GC+flow+I.pkl"
        ]
    ],
)
